...
使用深度学习模型 (**Hybrid Transformer-LSTM**: 混合 Transformer 和 LSTM) 来预测 WTI 原油每日收盘价格。项目包含完整的数据加载、特征工程、模型训练、验证和可视化流程。

**当前情况 (2026.01.29)**：
- **分支状态**: 当前位于 `Test` 分支 (原 `Rate` 分支)。
- **实验目的**: 尝试通过新的特征工程和训练策略解决模型预测滞后性的问题。
- **实验结果**: **未成功**。模型依然表现出一定的滞后性，且并未显著优于主分支。
- **详细分析**: 请参阅本目录下的 `实验分析报告.md` 获取完整的实验数据和原因分析。

> [!WARNING]
> 本分支的代码处于实验状态，仅供参考。详细的失败原因分析对于后续改进至关重要。
